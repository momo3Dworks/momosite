...
const SPACESHIP_FIXED_EMISSIVE_INTENSITY = 30.0;
...
const ScrollSurferScene: React.FC = () => {
  ...
  const initThreeScene = useCallback((currentMount: HTMLDivElement, loadingManager: THREE.LoadingManager) => {
    ...
    loadingMessageRef.current = "Loading MOMOSHIP...";
    gltfLoader.load('/models/MOMOSHIP.glb', (gltf) => {
      ...
      }, undefined, (error) => { console.error('Error loading MOMOSHIP model:', error); }
    );
  ...
  useEffect(() => {
    return () => {
      ...
      const currentMount = mountRef.current;
      if (currentMount && rendererDomElement && currentMount.contains(rendererDomElement)) {
